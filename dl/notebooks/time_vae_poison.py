...
# +
daily_profile = profile_sin(np.arange(0, 12 * 60 / 5, 1), lambda_min=9, lambda_max=10)
...
df = df_visitors.loc[df_visitors["data"] == "visitors"][["t", "count"]]
df["count"] = df["count"] + 2
...
if IS_RELOAD:
    checkpoint_path = "lightning_logs/time_vae_koisk/version_8/checkpoints/epoch=1999-step=170000.ckpt"
...
investigation_dl = DataLoader(
    dataset=DataFrameDataset(
        dataframe=df[["count"]], history_length=window_size, horizon=1, gap=0
    ),
    batch_size=400,
    shuffle=False,
)
